...
/*   Updated: 2019/10/03 18:14:01 by lbenard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_result	init_map_split(t_map *const self,
				char *map_file_str)
{
	if (!map_parse_size(self, wolf_select_flag(map_file_str, "size")))
	{
		free(map_file_str);
		return (throw_result_str("init_map()", "failed to parse size"));
	}
	if (!map_parse_map(self, wolf_select_flag(map_file_str, "map")))
	{
		free(map_file_str);
		return (throw_result_str("init_map()", "failed to parse map"));
	}
	if (!map_parse_player(self, wolf_select_flag(map_file_str, "player")))
	{
		free(map_file_str);
...
t_result	init_map(t_map *const self, const t_map_args *const args)
{
	...
	if (!map_parse_texture_list(self,
		wolf_select_flag(map_file_str, "textures")))
	{
		free(map_file_str);
		return (throw_result_str("init_map()", "failed to parse textures"));
	}
	if (!map_parse_block_list(self, wolf_select_flag(map_file_str, "blocks")))
	{
		free(map_file_str);
